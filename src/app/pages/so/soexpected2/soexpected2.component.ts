...
export class Soexpected2Component implements OnInit, AfterViewInit {
  ...
  constructor(public utilService: CommonUtilService,
    ...
    this.onSoTypeChanged = this.onSoTypeChanged.bind(this);
  }
  ...
  onShown(): void {
    ...
    if (this.popupFormData.soType === 'PURRTN') {
      // 매입반품일 경우 거래처
  ...
  async onPopupSearch(data): Promise<void> {
    ...
    this.dsPort = this.copyPort.filter(el => el.etcColumn1 === data.countrycd);
